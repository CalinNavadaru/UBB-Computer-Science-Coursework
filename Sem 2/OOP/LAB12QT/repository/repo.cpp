...
int Repository::cautaMedicamentIndice(const string &denumire, const string &producator, const string &substantaActiva, double pret) const {

    int k = 0;
    for (const auto& el: this->medicamente) {
        if (el.getDenumire() == denumire && el.getProducator() == producator && el.getPret() == pret && el.getSubstantaActiva() == substantaActiva) {
            break;
        }
        k++;
    }
    if (k == this->getNrMedicamente())
        throw RepositoryException("Medicamentul cautat nu exista!");
