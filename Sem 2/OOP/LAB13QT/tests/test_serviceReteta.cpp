...
void test_serviceReteta() {
    ...
    try {
        serviceReteta.addMedicament("asdsadsadasdasdasd");
        assert(false);
    ...
    try {
        serviceReteta.scrieInFisier("test.csv");
        assert(true);
    } catch (...) {
        assert(false);
    }
