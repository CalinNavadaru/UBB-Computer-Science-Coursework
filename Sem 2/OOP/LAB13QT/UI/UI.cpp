...
void UI::initLayout() {
    ...
    modellista = new ModelLista{nullptr, serviceCRUD};
    modelTable = new ModelTable{nullptr, serviceReteta};
    listaView = new QListView{};
    listaView->setModel(modellista);
    layoutTable->addWidget(listaView);
...
void UI::connectBtns() {
    ...
    connect(SearchButton, &QPushButton::clicked, [this](){
        ...
        if (inputDialog->exec()) {
            ...
            try {
                ...
                QMessageBox::information(this, "Cautare", "Medicamentul se afla pe linia " + QString::number(x + 1, 'g', 10) + "!");
            } catch(RepositoryException& e) {
                auto *windowEroare = new QMessageBox{};
                windowEroare->setWindowTitle(QString{"Date invalide"});
                windowEroare->setText( QString{e.getMsg().c_str()});
    ...
    connect(VeziCos, &QPushButton::clicked, [this](){
        auto cosWindow = new UiCos{*this, modelTable};
        adaugaObservator(cosWindow);
        cosWindow->show();
    });

    connect(VeziCosReadOnly, &QPushButton::clicked, [this](){
        auto cosWindow = new UiCosReadOnly{*this, modelTable};
        adaugaObservator(cosWindow);
        cosWindow->show();
    });
