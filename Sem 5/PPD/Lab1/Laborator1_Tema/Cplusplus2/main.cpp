...
auto matrix = new int[NMAX][MMAX];
auto result = new int[NMAX][MMAX];
auto result2 = new int[NMAX][MMAX];
auto conv = new int[nMAX][mMAX];
int N, M, n, m, p;

void initMatrix(int matrix[NMAX][MMAX], const int iStart, const int iFinal, const int jStart, const int jFinal,
                ifstream &f) {
...
void initConv(int matrix[nMAX][mMAX], const int iStart, const int iFinal, const int jStart, const int jFinal,
              ifstream &f) {
    for (int i = iStart; i <= iFinal; i++) {
        for (int j = jStart; j <= jFinal; j++) {
            f >> matrix[i][j];
...
void operation(int matrix[NMAX][MMAX], int conv[nMAX][mMAX], int result[NMAX][MMAX], const int i, const int j,
...
void sequential(int matrix[NMAX][MMAX], int conv[nMAX][mMAX], int result[NMAX][MMAX], const int n, const int m,
                const int N, const int M) {
...
void liniar(int matrix[NMAX][MMAX], int conv[nMAX][mMAX], int result[NMAX][MMAX], const int n, const int m, const int N,
            const int M, int start, int end) {
...
void coloana(int matrix[NMAX][MMAX], int conv[nMAX][mMAX], int result[NMAX][MMAX], const int n, const int m,
             const int N, const int M, int start, int end) {
...
int main(int argc, char **argv) {
    ...
    output << "dinamic " << N << " " << M << " " << n << " " << m  << " " << p <<   " secvential C++ " << elapsed_time_ms << endl;
    ...
    for (int i = 0; i < p; i++) {
        ...
        threads[i] = thread(liniar, ref(matrix), ref(conv), ref(result2), n, m, N, M, start + (n - 1) / 2,
                            end + (n - 1) / 2);
        start = end;
        end = start + N / p;
    }
    ...
    std::cout << elapsed_time_ms << "\n";
    output << "dinamic " << N << " " << M << " " << n << " " << m  << " " << p <<   " linie C++ " << elapsed_time_ms << endl;
    ...
    for (int i = 0; i < p; i++) {
        ...
        threads[i] = thread(coloana, ref(matrix), ref(conv), ref(result2), n, m, N, M, start + (m - 1) / 2,
                            end + (m - 1) / 2);
        start = end;
        end = start + M / p;
    }
    ...
    elapsed_time_ms = std::chrono::duration<double, std::nano>(t_end - t_start).count();
    std::cout << elapsed_time_ms << "\n";
    output << "dinamic " << N << " " << M << " " << n << " " << m  << " " << p <<   " coloane C++ " << elapsed_time_ms << endl;
    assert(comp(N, M, n, m) == true);
    ...
    for (int i = 0; i < p; i++) {
        ...
        threads[i] = thread(bloc, ref(matrix), ref(conv), ref(result2), n, m, N, M, startI + (n - 1) / 2,
                            endI + (n - 1) / 2,
    ...
    elapsed_time_ms = std::chrono::duration<double, std::nano>(t_end - t_start).count();
    std::cout << elapsed_time_ms << "\n";
    output << "dinamic " << N << " " << M << " " << n << " " << m  << " " << p <<  " bloc C++ " << elapsed_time_ms << endl;
    assert(comp(N, M, n, m) == true);
